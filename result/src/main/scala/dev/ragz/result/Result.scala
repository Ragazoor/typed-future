...
sealed trait Result[+E <: Throwable, +A] extends Awaitable[A] {
  self =>
  def untyped: StdFuture[A]

  def value: Option[Try[A]] = self.untyped.value

  def map[B](f: A => B)(implicit ec: ExecutionContext): Result[E, B] =
    Result(self.untyped.transform(_ map f))

  def flatMap[E2 >: E <: Throwable, B](f: A => Result[E2, B])(implicit ec: ExecutionContext): Result[E2, B] =
    Result(self.untyped.flatMap(f(_).untyped))
  ...
  def zipWith[E2 >: E <: Throwable, U, R](that: Result[E2, U])(f: (A, U) => R)(implicit
    ...
    Result(self.untyped.zipWith(that.untyped)(f))

  def catchAll[E2 >: E <: Throwable, A2 >: A](f: E => Result[E2, A2])(implicit
                                                                      ec: ExecutionContext
  ): Result[E2, A2] =
    Result[E2, A2] {
      self.untyped.transformWith {
        case Failure(e) if NonFatal(e) => f(e.asInstanceOf[E]).untyped
        case _                         => self.untyped
      }
    }
...
object Result {
  ...
  final def sequence[E <: Throwable, A](results: Seq[Result[E, A]])(implicit
    ...
    Result(StdFuture.sequence(results.map(_.untyped)))
